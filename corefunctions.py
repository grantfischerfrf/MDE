...
def matchHist(ref, image):
    ...
    if (len(image.shape) > 2) & (image.shape[2] > 1):
        ...
        matched_v = match_histograms(
            im_hsv[:, :, 2], ref_hsv[:, :, 2] , channel_axis=False
        )
        ...
        matched = match_histograms(image, ref, channel_axis=False)
    return matched
...
def mergeRectify(input_frames, cameras, grid):
    ...
    for k, (I, calib) in enumerate(zip(input_frames, cameras)):
        ...
        if calib.Ud.any() == "None":  #.Ud is a call for distorted U coordinates.
...
class XYZGrid(object):
    ...
    def initXYZ(self, xlims, ylims, dx, dy, z):
        ...
        if len(xlims) < 2:
            ...
            # xvec = np.arange(xlims[0], xlims[1], dx)

        if len(ylims) < 2:
            yvec = ylims
        else:
            yvec = np.arange(ylims[0], ylims[1] + dy, dy)
            # yvec = np.arange(ylims[0], ylims[1], dy)
...
class CameraData(object):
    ...
    def localTranformExtrinsics(self):
        ...
        # self.origin[2] = np.deg2rad(self.origin[2]) #TODO: this is happening twice
